...
const rows: Row[] = [
  // ── Canada ──
  // HOS: (Canada Sales Lead), RHO: Suzy Lawlor
...
async function main() {
  ...
  await prisma.salesAlignment.deleteMany();

  console.log(`Seeding ${rows.length} alignment rows...`);
  for (const row of rows) {
    await prisma.salesAlignment.create({ data: row });
  }

  console.log(`Done. ${rows.length} rows seeded.`);
}
